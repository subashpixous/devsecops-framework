...
Invariants enforced here and covered by unit tests:
  ...
  * A scanner failure, absence, partial result or malformed payload can only
    produce NOT_VERIFIED. It can never produce PASS.
...
# Language inference for coverage checks; deliberately generic.
_EXTENSION_LANGUAGE = {
    ...
    ".html": "html", ".htm": "html", ".css": "css", ".scss": "css", ".vue": "vue",
    ".svelte": "svelte", ".sql": "sql", ".sh": "shell", ".ps1": "powershell",
}

# Markup/styling languages are not expected to yield security findings, so their
# absence from a result set is not evidence of missing analysis.
...
class SecurityAssessment:
    """Complete evaluated state for one run."""
...
class StatusEngine:
    ...
    def _resolve_categories(
        ...
        for category in CATEGORY_REGISTRY:
            ...
            outcome.finding_count = len([f for f in category_findings if f.is_open])
            ...
            if untrustworthy:
                ...
                outcome.status = CATEGORY_NOT_VERIFIED
                outcome.reason = (
                    "Scanner did not complete successfully, so this category could not be verified."
                )
            ...
            if breaches or gate_failed:
                ...
                if gate_failed:
                    ...
                    parts.append(
                        ...
                        % (
                            " (" + ", ".join(c.get("metric", "?") for c in failing) + ")"
                            if failing
                            else ""
                        )
                    )
    ...
    def _evaluate_thresholds(self, findings: List[Finding]) -> List[ThresholdBreach]:
        relevant = [
            f
            for f in findings
            if f.is_open and self.policy.is_security_finding_category(f.category)
        ]
    ...
    def _security_status(self, outcomes: List[CategoryOutcome]) -> Tuple[str, List[str]]:
        ...
        if failed:
            ...
            unverified = [c for c in required_not_pass if c.status != CATEGORY_FAILED]
        ...
        rationale.append(
            "SECURITY = PASS for the controls implemented in the active phase only. Every required "
            "control executed successfully and no open finding breaches policy."
        )
    ...
    def _runtime_status(self, outcomes: List[CategoryOutcome]) -> Tuple[str, List[str]]:
        ...
        applicable = [c for c in runtime if c.status not in (CATEGORY_NOT_APPLICABLE,)]
        ...
        if untested or not tested:
            ...
            rationale.append(
                "RUNTIME_SECURITY = NOT_TESTED. No runtime security testing was performed against a "
                "running instance in this run."
            )
            return RUNTIME_NOT_TESTED, rationale

        rationale.append("RUNTIME_SECURITY = PASS for the runtime controls implemented in the active phase.")
    ...
    def _limitations(
            ...
            limitations.append(
                {
                    ...
                    "detail": (
                        ...
                        "This report covers only the categories implemented in phase %d."
                        % (
                            len(not_implemented),
                            ", ".join(sorted(c.title for c in not_implemented)),
                            self.policy.active_phase,
                        )
                    ),
                }
            )
        ...
        # Language coverage: a language present in the repository with zero findings
        # is ambiguous -- clean, or not analysed at all. Say so rather than assume.
        detected = {
            str(language)
            ...
            limitations.append(
                {
                    ...
                    "detail": (
                        ...
                        "This may mean the code is clean, or that the analyser did not scan the language "
                        "at all. Confirm lines-of-code per language on the analysis server before "
                        "treating these languages as covered."
                        % ", ".join(unconfirmed)
                    ),
                }
            )
        ...
        limitations.append(
            {
                ...
                "detail": (
                    ...
                    "vulnerability. 11 manual control areas remain untested and are listed in full in "
                    "the manual controls section."
                ),
            }
        )
        return limitations
    ...
    def evaluate(
        ...
        outcomes, breaches = self._resolve_categories(capabilities, scanner_results, findings, quality_gate)
        ...
        if build_status == BUILD_UNKNOWN:
            rationale.append(
                "BUILD status was not reported by the caller; recorded as UNKNOWN rather than assumed."
            )
        if not context.deployment_status_input:
            rationale.append(
                "DEPLOYMENT status was not reported by the caller; recorded as SKIPPED. Deployment state "
                "has no influence on the security verdict."
            )

        implemented = sorted(
            c.key for c in outcomes if c.status in (CATEGORY_PASS, CATEGORY_FAILED, CATEGORY_NOT_VERIFIED)
        )
        ...
        security_relevant = [
            f for f in findings if self.policy.is_security_finding_category(f.category)
        ]
        ...
        return SecurityAssessment(
            ...
            verdict_scope="PHASE_%d[%s]" % (self.policy.active_phase, ",".join(implemented) or "none"),
            ...
            limitations=self._limitations(capabilities, outcomes, findings, scanner_results),
